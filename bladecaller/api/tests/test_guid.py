...
class GuidTests(APITestCase):
    ...
    def test_get_guid(self):
        response = self.client.get('/create-guid/', format='json')
        self.assertEqual(response.status_code, 405)


    def test_post_guid(self):
        response = self.client.post('/create-guid/', {}, format='json')
    ...
    def test_post_guid_with_state(self):
        response = self.client.post('/create-guid/', {'state': 'wa'}, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertTrue('guid' in response.data.keys())
        self.assertTrue(response.data['guid'].startswith('wa-xx-'))

    def test_post_guid_with_state_and_job(self):
        response = self.client.post('/create-guid/', {'state': 'wa', 'jobType': 'load'}, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertTrue('guid' in response.data.keys())
